...
export default function Booking({Val,setV}){
    ...
    const s = useStates({
      ...
      useEffect(() => {
        (async () => {
          ...
          let screening = (await (await fetch(`/api/occupied_seats?screeningId=${Val.idValue}`)).json())[0];
      ...
      return s.seats.length === 0 ? null : <div className="screening-and-seats">
        ...
        <h2>{new Intl.DateTimeFormat('en-GB', {
          weekday: "long",
          year: "numeric",
          month: "long",
          day: "numeric",
